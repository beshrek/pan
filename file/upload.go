...
import (
	...
	"github.com/beshrek/pan/account"
	"github.com/beshrek/pan/conf"
	fileUtil "github.com/beshrek/pan/utils/file"
...
type SuperFile2UploadResponse struct {
	...
	PartSeq  string `json:"-"` //pcsapi PHP版本返回的是int类型，Go版本返回的是string类型
}
...
func (u *Uploader) Upload() (UploadResponse, error) {
	...
	sem := make(chan int, 1) //限制并发数，以防大文件上传导致占用服务器大量内存
